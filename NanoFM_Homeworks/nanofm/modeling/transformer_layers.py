...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        q, k, v = rearrange(self.qkv(x), "b l (t h d) -> t b h l d", t=3, h=self.num_heads)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        k, v = rearrange(self.kv(context), "b m (t h d) -> t b h m d", t=2, h=self.num_heads)
...
class Block(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, mlp_ratio: float = 4., use_bias: bool = False):
        ...
        self.mlp = Mlp(dim, hidden_features=mlp_hidden_dim, bias=use_bias) # TODO
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, mlp_ratio: float = 4., use_bias: bool = False):
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(dim, hidden_features=mlp_hidden_dim, bias=use_bias) # TODO MLP layer
