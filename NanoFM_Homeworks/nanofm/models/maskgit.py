...
class MaskGIT(nn.Module):
    ...
    def get_maskgit_schedule(self, mask: torch.BoolTensor, num_steps: int = 8) -> List[int]:
        ...
        schedule = [tokens_per_step] * (num_steps - 1)+ [total_tokens % num_steps + tokens_per_step]
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            _, topk_masked_indices = torch.topk(confidence, k=k, dim=0)
            selected_positions = masked_indices[topk_masked_indices]
