...
class VisionLanguageModel(nn.Module):
    ...
    def forward(self, input_ids, image, attention_mask=None, targets=None):
        ...
        if targets is not None:
            ...
            logits = output_token_embeddings if self.decoder.lm_use_tokens else self.decoder.head(output_token_embeddings)
    ...
    def generate(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        for i in range(max_new_tokens):
            ...
            if attention_mask is not None:
                ...
                attention_mask = torch.cat((attention_mask, next_attention), dim=1) # (vi)
    ...
    def generate_with_kv_cache(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        for i in range(1, max_new_tokens):
            ...
            if (next_token == self.cfg.lm_eos_token_id).all(): # stop the generation loop if the generated token == 2 (token id 2 is the EOS token we used during training)
                break
